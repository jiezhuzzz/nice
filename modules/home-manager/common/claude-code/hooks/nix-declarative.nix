# The Nix counterpart of the pip rule (uv-only.nix), and it exists for the
# same reason: an imperative install is the one thing that puts software on
# this machine without a line in this flake describing it. It also hides —
# nothing in a `git status` shows that `nix-env -i` ever ran, so the drift is
# only found much later, on a host that was supposed to be reproducible.
#
...
in
  mkGuardHook "claude-nix-declarative-hook" (
