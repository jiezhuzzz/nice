...
# own user but must share files so *arr apps can hardlink Transmission's
...
_: {
  ...
  # roots are root:media (the *arr apps write inside them via group-write, they
  # need not own the root); download dirs are transmission:media.
